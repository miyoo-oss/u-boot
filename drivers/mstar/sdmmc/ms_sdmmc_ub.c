...
static int mstar_send_cmd(struct udevice *dev, struct mmc_cmd *cmd, struct mmc_data *data)
{
    ...
    {
        if(nErr == -ECOMM)
            printf("_[sdmmc_%u] Warn: #Cmd_%u (0x%08X)=>(E: 0x%04X)(S: 0x%08X)__(L:%u)\n", eSlot, u8CMD, u32Arg, (U16_T)eErr, cmd->response[0], eRspSt->u32ErrLine);
        else
            printf("_[sdmmc_%u] Err: #Cmd_%u (0x%08X)=>(E: 0x%04X)(S: 0x%08X)__(L:%u)\n", eSlot, u8CMD, u32Arg, (U16_T)eErr, cmd->response[0], eRspSt->u32ErrLine);
    }
    else if(data && nErr)
    {
        if(nErr == -ECOMM)
            printf(">> [sdmmc_%u] Warn: #Cmd_%u (0x%08X)=>(E: 0x%04X)(S: 0x%08X)__(L:%u)(B:%u)\n", \
                  eSlot, u8CMD, u32Arg, (U16_T)eErr, cmd->response[0], eRspSt->u32ErrLine, u16BlkCnt);
        else
            printf(">> [sdmmc_%u] Err: #Cmd_%u (0x%08X)=>(E: 0x%04X)(S: 0x%08X)__(L:%u)(B:%u)\n", \
                  eSlot, u8CMD, u32Arg, (U16_T)eErr, cmd->response[0], eRspSt->u32ErrLine, u16BlkCnt);
...
/*----------------------------------------------------------------------------------------------------------
 *
 * Function: mstar_set_ios
 *     @author jeremy.wang (2013/7/19)
 * Desc: Set IO bus Behavior
 *
 * @param mmc : mmc structure pointer
 ----------------------------------------------------------------------------------------------------------*/
static int mstar_set_ios(struct udevice *dev)
{
    ...
    pr_sd_main("_[sdmmc_%u] mstar_set_ios =>", (U32_T)eSlot);
 ...
 * @return int  : 0 (pass), else (fail
 ----------------------------------------------------------------------------------------------------------*/
static int mstar_core_init(struct mmc *mmc)
{
    SlotEmType eSlot = EV_SDMMC1;
    printf("\n_[sdmmc_%u] mstar_core_init\n", (U32_T)eSlot);
...
//###########################################################################################################
    if(1)
    {
        printf("_[sdmmc_%u] [sdio ip]\n", (U32_T)eSlot);

        _SetPAD(eSlot, EV_IP_FCIE1, EV_PFCIE5_SDIO, EV_PAD1);
    }
    else
    {
        printf("_[sdmmc_%u] [fcie ip]\n", (U32_T)eSlot);
...
//###########################################################################################################
    ...
    printf("_[sdmmc_%u] Turning on \n", (U32_T)eSlot);


    _SetPower(eSlot, EV_POWER_ON);
    printf("_[sdmmc_%u] Power on \n", (U32_T)eSlot);
...
static int mstar_mmc_ofdata_to_platdata(struct udevice *dev)
{
    ...
    IPEmType eIP     = ge_IPSlot[EV_SDMMC1];
	...
	if (ret < 0) {
		printf("failed to parse host caps\n");
		return ret;
	}

	cfg->f_min = Hal_CARD_FindClockSetting(eIP, 400000, 0, 0);;
	cfg->f_max = gu32_MaxClkSlot[0];

	cfg->voltages = MMC_VDD_32_33| MMC_VDD_31_32| MMC_VDD_30_31| MMC_VDD_29_30 | MMC_VDD_28_29 | MMC_VDD_27_28;
