...
#ifdef CONFIG_CMD_SF
#include <spi_flash.h>
int _spi_flash_read(struct mtd_info *flash, loff_t offset, size_t len,
                         size_t *retlen, u_char *buf)
{
...
int _spi_flash_write(struct mtd_info *flash, loff_t offset, size_t len,
                          size_t *retlen, const u_char *buf)
{
...
int _spi_flash_erase(struct mtd_info *flash, struct erase_info *instr)
{
    u32 ret = -EIO;
    u32 u32ErasedSize;
    u32 starttime = get_timer(0);
    ...
    if(instr->addr % FLASH_BLOCK_SIZE)
    {
        u32ErasedSize = FLASH_BLOCK_SIZE - instr->addr%FLASH_BLOCK_SIZE;
        if (instr->len < u32ErasedSize)
        {
            u32ErasedSize = instr->len;
        }
        if(MDrv_SERFLASH_SectorErase(instr->addr, instr->addr+u32ErasedSize-1))
        {
            instr->addr += u32ErasedSize;
            instr->len -= u32ErasedSize;
            ret = 0;
        }
        else
        {
    ...
    if(instr->len >= FLASH_BLOCK_SIZE)
    {
        u32ErasedSize = instr->len - instr->len%FLASH_BLOCK_SIZE;

        if( MDrv_SERFLASH_AddressErase(instr->addr, u32ErasedSize, 1))
        {
            instr->addr += u32ErasedSize;
            instr->len -= u32ErasedSize;
            ret = 0;
...
*/
    ...
    flash->mtd.flags = MTD_CAP_NORFLASH;
    MDrv_SERFLASH_DetectSize((MS_U32*)&flash->mtd.size);
    flash->mtd._erase = _spi_flash_erase;
    flash->mtd._read = _spi_flash_read;
    flash->mtd._write = _spi_flash_write;
